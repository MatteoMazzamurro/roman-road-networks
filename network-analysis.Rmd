...
Modern road data. The original ESRI world road network can be downloaded from https://www.arcgis.com/home/item.html?id=83535020ce154bd5a498957c159e3a99. 
Use 7-zip to unzip the lpk file and place it in the data folder for this project. 
Then read the .gdb as an sf by running the following.
```{r load modern road data}
#Remark: BEFORE running this line, download the data as detailed above. This is the resulting local path.  
...
# exclude italian regions
edge_betweenness_road_type_provinces_no_Italy_region_df <- edge_betweenness_road_type_provinces_df[
      !str_detect(edge_betweenness_road_type_provinces_df$region,"Regio"),
    ]
edge_betweenness_road_type_provinces_no_Italy_region_df <- edge_betweenness_road_type_provinces_no_Italy_region_df[
      !edge_betweenness_road_type_provinces_no_Italy_region_df$region=="Roma",
    ]

# Boxplot
boxplot_betweenness_by_province_and_road_type <- ggplot(
    edge_betweenness_road_type_provinces_no_Italy_region_df,
    aes(
      x = reorder(region, median_value), 
      y = edge_betweenness, 
      fill = Type)) +
...
Compute the probability that a randomly chosen main road has larger edge betweenness than a randomly chosen secondary road.
```{r probability that a randomly chosen main road has larger edge betweenness than a randomly chosen secondary road}
# function to compare probability that a randomly chosen main road has larger edge betweenness than a randomly chosen secondary road
prob_larger_edge_betweenness <- function(main, secondary) {
  compared_betweenness <- outer(main, secondary, ">")
  mean(compared_betweenness)
}

# compute interquartile range for the betweenness
edge_betweenness_road_type_provinces_stats_df <- edge_betweenness_road_type_provinces_no_Italy_region_df %>% 
  group_by(region, Type) %>%
  summarise(
    median = median(edge_betweenness),
    iqr = IQR(edge_betweenness),
    edge_betweenness = list(edge_betweenness),
    .groups = "drop"
  )

# transform into wide format and omit regions with only one kind of roads
edge_betweenness_road_type_provinces_stats_wide_df <- edge_betweenness_road_type_provinces_stats_df %>%
  pivot_wider(
    names_from = Type,
    values_from = c(median, iqr, edge_betweenness)
) %>% na.omit()
  
# ratio of medians
edge_betweenness_road_type_provinces_stats_wide_df$median_ratio <-edge_betweenness_road_type_provinces_stats_wide_df$`median_Main Road`/edge_betweenness_road_type_provinces_stats_wide_df$`median_Secondary Road`

# apply function
edge_betweenness_road_type_provinces_stats_wide_df$probability_betweenness <-
  mapply(
    prob_larger_edge_betweenness,
    edge_betweenness_road_type_provinces_stats_wide_df$`edge_betweenness_Main Road`,
    edge_betweenness_road_type_provinces_stats_wide_df$`edge_betweenness_Secondary Road`
  )

# modify province map to include Italia
Italia_sf <- st_union(provinces_sf[str_detect(provinces_sf$province,"Regio"),])
provinces_and_Italia_sf <- provinces_sf %>%
  mutate(
    region  = ifelse(
      province %in% provinces_sf$province[str_detect(provinces_sf$province,"Regio")],
      "Italia",
      province
      )) %>%
  group_by(region) %>%
...
# add data to sf for choropleth
provinces_and_Italia_sf <- provinces_and_Italia_sf %>% 
  left_join(edge_betweenness_road_type_provinces_stats_wide_df, by = "region")
```

Plot a choropleth showing the probability that a randomly chosen main road has larger edge betweenness than a randomly chosen secondary road.
```{r choropleth of probability main road vs secondary road}
# reduce resolution before creating choropleth
provinces_and_Italia_sf_small <- provinces_and_Italia_sf |>
  st_transform(3857) |>
  st_simplify(dTolerance = 1000) |>
  st_transform(4326)
world_nb_small_sf <- world_nb_sf |>
  st_transform(3857) |>
  st_simplify(dTolerance = 1000) |>
  st_transform(4326)

# plot choropleth
choropleth_betweenness <- ggplot(provinces_and_Italia_sf_small) +
  
  # background map
  geom_sf(data = world_nb_small_sf, 
          color = "lightgray",
          fill = "lightgray")+
  
  # regions
  geom_sf(
    aes(fill = probability_betweenness - 0.5),
    color = "white",
    linewidth = 0.2
  ) +
  ...
  labs(
    title = "Edge betweenness by road type",
    #subtitle = "Probability that a main road has higher betweenness than a secondary road"
    subtitle = "P(main > secondary) -0.5"
  )

# extract legend
choropleth_betweenness_legend <- get_legend(choropleth_betweenness)

# figure without legend
choropleth_betweenness_nolegend <- choropleth_betweenness + theme(legend.position = "none")

# save figures (with and without legend)
ggsave("choropleth_betweenness.svg", choropleth_betweenness, width = 180, height = 74, units = "mm")
ggsave("choropleth_betweenness_legend.svg", choropleth_betweenness_legend, width = 88, height = 74, units = "mm")
ggsave("choropleth_betweenness_no_legend.svg", choropleth_betweenness_nolegend, width = 88, height = 74, units = "mm")
ggsave("choropleth_betweenness.jpg", choropleth_betweenness, width = 180, height = 74, units = "mm")
```

Add a lollipop graph as a companion to the choropleth
```{r lollipop for betweenness}
# rewrite data in suitable form 
lollipop_data <- edge_betweenness_road_type_provinces_stats_wide_df %>%
  mutate(
    divergence = probability_betweenness - 0.5,
    direction  = if_else(divergence >= 0, "Main dominates", "Secondary dominates"),
    region_name = fct_reorder(region, divergence)
  )

# define plot
lollipop_plot <- ggplot(lollipop_data, aes(x = divergence, y = region_name, color = direction)) +

  # draw a vertical reference line at 0
  geom_vline(xintercept = 0, color = "gray40", linewidth = 0.4, linetype = "dashed") +

  # create sticks
  geom_segment(
    aes(x = 0, xend = divergence, y = region_name, yend = region_name),
    linewidth = 0.1
  ) +
  ...
  # colour consistent with your choropleth
  scale_color_manual(
    values = c("Main dominates"      = "#D95F02",
               "Secondary dominates" = "#2C7BB6"),
    name   = NULL
  ) +
  ...
  scale_x_continuous(
    ...
    labels = scales::label_number(style_positive = "plus")  # shows +0.1, -0.1 etc.
  ) +

  # labels
  labs(
    x        = "P(main > secondary) − 0.5",
    y        = NULL,
    title    = "Edge betweenness by road type",
    subtitle = "Positive = main roads tend to have higher betweenness"
  ) +
...
ggsave("choropleth_companion.svg", lollipop_plot, width = 88, height = 74, units = "mm")
```

Same as above, but horizontal
```{r lollipop plot horizontal}
# define plot
lollipop_plot_hor <- ggplot(lollipop_data, aes(x = region_name, y = divergence, color = direction)) +
  ...
  # create sticks
  geom_segment(
    aes(x = region_name, xend = region_name, y = 0, yend = divergence),
    linewidth = 0.3
  ) +
...
ggsave("choropleth_companion_hor.svg", lollipop_plot_hor, width = 88, height = 74, units = "mm")
ggsave("choropleth_companion_hor_big.svg", lollipop_plot_hor, width = 180, height = 74, units = "mm")
ggsave("choropleth_companion_hor.jpg", lollipop_plot_hor, width = 88, height = 74, units = "mm")
ggsave("choropleth_companion_hor_big.jpg", lollipop_plot_hor, width = 180, height = 74, units = "mm")
```
...
for (gu in geographical_units) {
  ...
  distance_vec <- distance_capital_to_Empire_nodes_l[[gu]]

  # order nodes by distance
  idx <- order(distance_vec)

  # sorted node attributes
  v_sorted <- vertex_attr_Empire_df[idx, ]

  rows_gu <- capital_centrality_Empire$province == gu

  # closest distances
  capital_centrality_Empire[rows_gu, "distance"] <- distance_vec[idx][1:5]

  # centralities
  capital_centrality_Empire[rows_gu, "betweenness"] <- v_sorted$betweenness[1:5]
  capital_centrality_Empire[rows_gu, "degree"] <- v_sorted$degree[1:5]
  capital_centrality_Empire[rows_gu, "closeness"] <- v_sorted$closeness[1:5]
  capital_centrality_Empire[rows_gu, "eigenvector"] <- v_sorted$eigenvector[1:5]

  # empirical quantiles
  capital_centrality_Empire[rows_gu, "betweenness_quantile"] <-
    ecdf(v_sorted$betweenness)(v_sorted$betweenness[1:5])

  capital_centrality_Empire[rows_gu, "degree_quantile"] <-
    ecdf(v_sorted$degree)(v_sorted$degree[1:5])

  capital_centrality_Empire[rows_gu, "closeness_quantile"] <-
    ecdf(v_sorted$closeness)(v_sorted$closeness[1:5])

  capital_centrality_Empire[rows_gu, "eigenvector_quantile"] <-
    ecdf(v_sorted$eigenvector)(v_sorted$eigenvector[1:5])
}

#write.csv(
#  capital_centrality_Empire,
#  "results/capital_centrality_Empire.csv",
#)
```
