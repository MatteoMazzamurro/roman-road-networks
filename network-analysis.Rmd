...
It includes the computation of classic global properties, geometric properties, and centrality. It compares the network with geometric models, and assesses the robustness of the results when a certain percentage of uncertain roads is omitted.
## Preliminaries
...
Add global clustering coefficient and average path length to identify distributed, linear and centralised networks.
...
### Other incomplete centralisation experiments
Explore centralisation of main connections (i.e., those with high betweenness centrality). 
We identify these based on interquartile range
```{r high betweenness edges}
# extract main edges function
main_edges_f <- function(net){
  # extract edge betweenness
  eb <- E(net)$edge_betweenness
  
  # Calculate Q3 
  Q3 <- quantile(eb, 0.75)
  
  # Extract main edges
  main_edges <- E(road_networks_l$Britannia)[eb >= Q3]
  
  # 
}

# extract network
net_ex <- road_networks_l[[gu]]


# extract betweenness values
eb <- road_networks_edge_betweenness_l[[gu]]
...
# Extract main edges
main_edges <- E(road_networks_l$Britannia)[eb >= Q3]
plot(main_edges$geometry)

sub_net <- 


# Alternatively, use interquartile range
#Q1 <- quantile(eb, 0.25)
#IQR <- Q3 - Q1
...
Find smallest connected network with high betweenenss
```{r}
eb_mst_f <- function(net){
  mst_graph <- mst(net, weights = 1/E(net)$edge_betweenness)
}
eb_mst_l <- lapply(road_networks_l,eb_mst_f)
eb_mst_entropy_l <- lapply(eb_mst_l, normalised_shannon_entropy_f) 
mst_entropy_df <- do.call(rbind,eb_mst_entropy_l) %>% as.data.frame() %>% setNames("entropy")
